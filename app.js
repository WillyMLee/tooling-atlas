...
const skills = [
  ...
  },
  {
    ...
    outcome: "A compact routing packet that explains what loads, what stays out, and when to re-route.",
    steps: [["Describe the task shape", "Classify scope, work shape, side effects, and whether improvement or reuse is part of the outcome."], ["Start direct", "Add a skill only when its description provides a method the task otherwise lacks."], ["Order specialists", "Route, prepare, execute, and improve without loading improvement work into execution."], ["Preserve the boundary", "Record activation metadata and re-route only if task shape or authorization materially changes."]],
    safeguards: ["Honor explicitly named skills", "Never broaden authorization", "Do not claim unmeasured gains"],
    example: { scenario: "A multi-part website workflow needs research, one implementation owner, browser QA, and a reusable skill only if the pattern succeeds.", pattern: "Route with Orchestration, execute browser verification with Web Interaction Loop, then evaluate and implement the skill after completion.", deliverable: "A use-now and after-completion packet with explicit skips and stop conditions." },
    measures: ["Correct first skill selection", "Unnecessary activations avoided", "Authorization preserved"],
    sourcePath: "skills/route-skills/SKILL.md",
    sources: [["OpenAI — Build skills", "https://learn.chatgpt.com/docs/build-skills"], ["Atlas routing policy", "https://github.com/WillyMLee/tooling-atlas/blob/main/skills/route-skills/references/routing-policy.json"]],
  },
  {
    slug: "implement-skills", name: "Skill Implementer", status: "Experimental · unmeasured",
    summary: "Turn a repeated or explicitly requested workflow into the smallest validated, installable, and measurable Codex skill package.",
    trigger: "A successful method should become reusable, an existing skill overlaps or is stale, or the user explicitly asks to create or improve a skill.",
    avoid: "A one-off workflow contains private or transient details and the user did not request a project-local skill.",
    outcome: "A versioned skill package with validation evidence, install state, and a defined next eval.",
    steps: [["Define triggers", "Capture concrete trigger and non-trigger examples, useful output, ownership, and authority boundaries."], ["Update before adding", "Search for the closest existing skill and create a new package only for a distinct job."], ["Build and validate", "Initialize the package, keep context lean, test deterministic scripts, and run structural checks."], ["Link and measure", "Install only when authorized, register the module, and keep performance claims unmeasured until matched runs exist."]],
    safeguards: ["Do not package private content", "Do not silently overwrite personal skills", "Do not promote without validation"],
    example: { scenario: "Several projects repeat the same research-to-design reconstruction workflow with the same failure modes.", pattern: "Extract the reusable method, update an adjacent skill if one exists, validate its metadata and scripts, then link it into Codex.", deliverable: "A canonical SKILL.md package plus trigger cases, validation results, and an eval contract." },
    measures: ["Validated skill acceptance", "Trigger precision", "Script and forward-test pass rate"],
    sourcePath: "skills/implement-skills/SKILL.md",
    sources: [["OpenAI — Build skills", "https://learn.chatgpt.com/docs/build-skills"], ["Atlas implementation contract", "https://github.com/WillyMLee/tooling-atlas/blob/main/skills/implement-skills/references/implementation-contract.md"]],
  },
];
...
const captureFragments = (site, spec) => [
  { file: "01-header", label: "Header + wayfinding", detail: "Global header and navigation", region: "x 0–100% · y 0–23%", role: "Record the identity lockup, primary routes, active state, profile count, theme action, and the rule separating navigation from content." },
...
const renderSourceCapture = (site, spec, compact = false) => `<figure class="source-capture ${compact ? "is-compact" : "is-large"}">
  ...
  ${compact ? `<span class="capture-badge">Actual source frame</span>` : `<div class="capture-markers" aria-hidden="true"><i style="--x:8%;--y:8%">01</i><i style="--x:24%;--y:45%">02</i><i style="--x:78%;--y:48%">03</i></div>`}
...
function renderOverview() {
  ...
  view.innerHTML = `<div class="page">
    ...
    <section class="content-section">
      <div class="section-heading"><div><span class="section-index">01 / Verified design research</span><h2>Design gallery</h2></div><p>Find a reference by task shape, scan three real page or product states, then open the profile for component crops, build order, and provenance.</p></div>
      ...
      <div class="gallery-method"><article><span>01 / Find</span><strong>Start with the task</strong><p>Search the component, archetype, theme, or interaction you actually need.</p></article><article><span>02 / Inspect</span><strong>Whole page, then parts</strong><p>Separate page sequence, alternate product states, and opening-frame anatomy.</p></article><article><span>03 / Rebuild</span><strong>Hand the agent a brief</strong><p>Use the hierarchy, dimensions, build order, cautions, and source files as constraints.</p></article></div>
...
function renderDesign(site) {
  ...
  view.innerHTML = `<article class="page">
    ...
    <nav class="design-profile-nav" aria-label="Profile sections"><button type="button" data-scroll-target="page-anatomy">Page frames</button><button type="button" data-scroll-target="component-anatomy">Component anatomy</button><button type="button" data-scroll-target="build-spec">Build specification</button><a href="${site.url}" target="_blank" rel="noreferrer">Live source ↗</a></nav>
    <section class="design-research-brief"><article><span>Use this reference when</span><p>${escapeHtml(site.pattern)}</p></article><article><span>Study these decisions</span><p>${escapeHtml((site.brandElements || []).join(" · "))}</p></article><article><span>Avoid copying blindly</span><p>${escapeHtml(site.caution)}</p></article><aside><strong>${captureSet.frames.length + captureFragments(site, spec).length} verified visuals</strong><span>${captureSet.frames.length} desktop frames · ${captureFragments(site, spec).length} component crops</span></aside></section>
...
function renderSkillsIndex() {
  ...
  const recipes = [
    ["Broad source audit", "Context Budget", "Keep a compact evidence map; add Orchestration only if the audit splits into distinct workstreams."],
    ["Large independent read set", "Batch Tool Calls", "Batch reads after independence and threshold checks pass."],
    ["Visible browser workflow", "Web Interaction Loop", "Observe, act, and verify; add Orchestration only when this is one part of a larger workflow."],
    ["Repeated successful workflow", "Skill Implementer", "Package the reviewed method after completion, then validate and benchmark it before promotion."],
  ];
  view.innerHTML = `<div class="page module-index-page">${pageHead("AGENT MODULES / ORCHESTRATION", "Start with the task shape.<br><em>Add only what helps.</em>", "Orchestration is the coordinator. Skills are optional specialists with explicit inputs, outputs, and evidence—not a stack that runs on every task.")}
    ...
    <section class="task-shape-guide" id="module-recommender"><div class="compact-section-head"><span class="field-label">Smart skills router / prototype</span><h2>Most tasks need no module or one specialist.</h2><p>Answer three questions. The recommendation separates what helps during the task from evaluation or skill implementation afterward.</p></div><form class="module-recommender"><label><span>Task scope</span><select name="scope"><option value="small">Small, bounded</option><option value="broad">Broad, many-source</option><option value="multi">Multi-part workflow</option></select></label><label><span>Work shape</span><select name="shape"><option value="coupled">Coupled decisions</option><option value="independent">Many independent reads</option><option value="browser">Visible browser state</option></select></label><label><span>System outcome</span><select name="measure"><option value="no">Complete the task</option><option value="yes">Measure an improvement</option><option value="skill">Create or update a skill</option></select></label></form><div class="module-recommendation" aria-live="polite"></div><div class="task-recipes">${recipes.map(([shape,choice,reason]) => `<article><span>${shape}</span><strong>${choice}</strong><p>${reason}</p></article>`).join("")}</div></section>
  ...
  const form = view.querySelector(".module-recommender");
  const output = view.querySelector(".module-recommendation");
  const moduleLink = (slug) => { const skill = skills.find((item) => item.slug === slug); return `<a href="#skill/${slug}">${escapeHtml(skill?.name || slug)}</a>`; };
  const updateRecommendation = () => {
    ...
    const active = [];
    if (scope === "multi") active.push("orchestration-plan");
    if (scope === "broad") active.push("context-budget");
    if (shape === "independent") active.push("batch-tool-calls");
    if (shape === "browser") active.push("web-interaction-loop");
    const unique = [...new Set(active)];
    const later = measure === "skill" ? ["eval-improvement-loop", "implement-skills"] : measure === "yes" ? ["eval-improvement-loop"] : [];
    output.innerHTML = `<div><span>Use now</span><strong>${unique.length ? unique.map(moduleLink).join(" + ") : "Work directly"}</strong><p>${unique.length ? "Each specialist gets a bounded role. Orchestration appears only for a genuinely multi-part dependency graph." : "The task is bounded and coupled, so coordination overhead is unlikely to help."}</p></div><div><span>After completion</span><strong>${later.length ? later.map(moduleLink).join(" + ") : "No improvement module"}</strong><p>${measure === "skill" ? "Evaluate the reviewed method, then create or update the smallest validated skill package." : measure === "yes" ? "Compare a matched baseline and candidate only after success criteria are fixed." : "Capture normal proof of completion; add an eval only when a reusable decision is at stake."}</p></div>`;
  };
...
function renderControlTower() {
  ...
  const awaitingSummaries = evalSummaries.filter((summary) => summary.pairedRuns === 0);
